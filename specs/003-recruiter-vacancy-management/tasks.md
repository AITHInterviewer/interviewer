...
[[002-batch-evaluation-contour]] не пишет `Evaluation` сам) убедиться, что таблица не
...
- US3 читает `Evaluation`/`RecruiterDecision`, которые эта фича только объявляет как схему (T010, T011) — реальные значения пишет [[002-batch-evaluation-contour]]; до его готовности US3 демонстрируется на данных, засеянных вручную (`quickstart.md`)
...
4. US3 → демо «таблица + отчёт» (на засеянных данных до готовности [[002-batch-evaluation-contour]])
...
- US3 сознательно не блокируется на [[002-batch-evaluation-contour]] — это отдельная
